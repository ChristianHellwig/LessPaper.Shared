using System;
using System.Collections.Generic;
using System.Text;
using System.Threading.Tasks;
using LessPaper.Shared.Interfaces.General;

namespace LessPaper.Shared.Interfaces.Database.Manager
{
    public interface IDbUserManager
    {
        /// <summary>
        /// Add a new user to the database
        /// </summary>
        /// <param name="userId">New user id</param>
        /// <param name="rootDirectoryId">New id of the root directory (will be created)</param>
        /// <param name="email">Email address of the user</param>
        /// <param name="hashedPassword">Hashed password</param>
        /// <param name="salt">Salt used to generate the hash</param>
        /// <returns></returns>
        Task<bool> InsertUser(string userId, string rootDirectoryId, string email, string hashedPassword, string salt);

        /// <summary>
        /// Delete user including all directories, files and every other related information
        /// </summary>
        /// <param name="requestingUserId">Id of the requesting user</param>
        /// <param name="userId"></param>
        /// <returns></returns>
        Task<bool> DeleteUser(string requestingUserId, string userId);

        /// <summary>
        /// Receive basic information about a user.
        /// Such as the hashed password or the root directory id
        /// </summary>
        /// <param name="requestingUserId">Id of the requesting user</param>
        /// <param name="userId"></param>
        /// <returns></returns>
        Task<IMinimalUserInformation> GetBasicUserInformation(string requestingUserId, string userId);
    }
}
